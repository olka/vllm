...
class Scheduler(SchedulerInterface):
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    ...
                    logger.error("Full preemption: req %s",
            ...
            if request_id in self._rolled_back_req_ids:
                # Rolled-back request: worker needs ALL block IDs
                # (not just newly allocated) to rebuild block table.
    ...
    def _make_cached_request_data(
        ...
        for idx, req in enumerate(itertools.chain(running_reqs, resumed_reqs)):
            ...
            elif req_id in self._rolled_back_req_ids:
                # Page-fault rollback: treat like a resumed request so the
                # worker removes from persistent batch and re-adds with
                # correct (truncated + newly allocated) block table.
                resumed_req_ids.add(req_id)
            if not scheduled_in_prev_step or req_id in self._rolled_back_req_ids:
